...
train_file_path = '../Datasets/train_multiclass.csv'
test_file_path = '../Datasets/test_multiclass.csv'

# train_file_path = 'adasyntrain.csv'
# test_file_path = 'adasyntest.csv'
...
#
# """3rd approach Condensed Nearest Neighbours"""
# print(sorted(Counter(y_train).items()))
...
# Undersample majority class first and then oversample
...
# clustering undersampling for majority class
# from sklearn.cluster import MiniBatchKMeans
# from imblearn.under_sampling import ClusterCentroids
# print(sorted(Counter(y_train).items()))
# cc = ClusterCentroids(estimator=MiniBatchKMeans(n_init=1, random_state=0), random_state=42, sampling_strategy='majority')
...
# save_dataframe(X_train_adasyn, y_train_adasyn, X_test, y_test, X_train_adasyn.columns, 'adasyn')
...
# # print(sorted(Counter(y_train).items()))
# nm = NearMiss(version=3, sampling_strategy='all')
# X_res, y_res = nm.fit_resample(X_train_adasyn, y_train_adasyn)
# print(sorted(Counter(y_res).items()))

# # Cleaning rule
# from imblearn.under_sampling import NeighbourhoodCleaningRule
# #print(sorted(Counter(y_train).items()))
# ncr = NeighbourhoodCleaningRule(sampling_strategy='all', n_jobs=-1)
# X_res, y_res = ncr.fit_resample(X_train_adasyn, y_train_adasyn)
# print(sorted(Counter(y_res).items()))
#
#
# """Borderline smote"""
...
# save_dataframe(X_resampled, y_resampled, X_test, y_test, X_resampled.columns, 'borderlinesmote')
# del X_resampled, y_resampled

""" Undersample already oversampled datasets"""
# from imblearn.under_sampling import ClusterCentroids
# print(sorted(Counter(y_train).items()))
